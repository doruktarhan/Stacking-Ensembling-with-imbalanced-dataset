...
fl1 = FocalLoss(alpha=.3, gamma=2)
fl2 = FocalLoss(alpha=0.75, gamma=2.0)
...
#bu calismayan multi icin binarye cevirince calisiyor
'''
...
'''
clf = OneVsRestLightGBMWithCustomizedLoss(loss=fl1)
...
y_pred = clf.predict_proba(x_vali)

logloss = log_loss(y_vali, y_pred)

print()
print(f"Test's logloss: {logloss:.5f}")